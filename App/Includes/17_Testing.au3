...
;			DirMove($g_DownDir & '\Not Valid', $g_RemovedDir & '\IE-Downloads'&@YEAR&@MON&@MDAY, 1)
;			_Process_SetScrollLog(StringFormat(_GetTR($Message, 'L16'), $g_RemovedDir & '\IE-Downloads-'&@YEAR&@MON&@MDAY)); => files can be found at X
;		Else; restore files
;			FileMove($g_DownDir & '\Not Valid\*.*', $g_DownDir & '\')
;		EndIf
;	Else
...
Func _Test_Get_EET_Mods(); called by _Tree_EndSelection() just before starting an installation
	Local $BG1EE_Mods='WeiDU|WeiDU64|bwinstallpack|bwtrimpack|bwfixpack|BWS-FR-Fixpack|'; trailing | is needed
